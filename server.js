...
const EDITION="Hedge Fund Terminal";
const BUILD="2026.07.06-HFT1";
const API_VERSION="12.0.0-hedge-fund-terminal";
...
async function fetchText(url,timeout=12000){
  ...
  try{
    const res=await fetch(url,{signal:ctrl.signal,headers:{"accept":"application/json,text/plain,*/*","user-agent":"Mozilla/5.0 crypto-scanner-v12"}});
...
function buildSectors(coins){
  ...
  return Object.entries(map).map(([sector,items])=>{
    ...
    const volume=items.reduce((s,x)=>s+x.volume,0);
    return{sector,count:items.length,avg24:round(a24,2),avg7:round(a7,2),strength:clamp(Math.round(50+a24*4+a7*1.5),0,100),volume};
...
function tradePlan(c){
  ...
  const vol=Math.max(Math.abs(c.change24h||0),range,2);
  let rr=c.change24h>0&&c.change24h<8?2:c.change24h>=8?1.45:c.change24h<0?1.55:1.75;
  if(c.change7d>0)rr+=.12; if(vol>=2&&vol<=10)rr+=.15; if(vol>16)rr-=.25;
  rr=clamp(rr,1.1,2.6);
  const atr=c.price*Math.max(vol/100,.018), entryHigh=c.price, entryLow=Math.max(c.price-atr*.35,c.price*.985), sl=Math.max(c.price-atr*.75,c.price*.97);
  return{volatility:vol,atr,rr,entryLow,entryHigh,sl,tp1:c.price+(c.price-sl)*1.2,tp2:c.price+(c.price-sl)*2,tp3:c.price+(c.price-sl)*3};
}
...
function aiScore(c){
  ...
  let total=0,xai=[];
  for(const[k,weight]of Object.entries(w)){const contribution=c[k]*weight;total+=contribution;xai.push({component:k,score:round(c[k],1),weight,contribution:round(contribution,1)});}
...
function confidence({score,comps,penalties,providerQuality}){
  const vals=Object.values(comps), dispersion=Math.max(...vals)-Math.min(...vals);
  let c=score*.62+comps.risk*.16+comps.liquidity*.10+providerQuality*.12;
  if(dispersion>65)c-=12; c-=Math.min(18,(penalties||[]).length*5);
  return clamp(Math.round(c),0,98);
}
function quantMetrics({score,conf,rr,volatility,regime,components}){
  const winProb=clamp(Math.round(35+score*.34+conf*.22-(regime.risk==="HIGH"?12:0)-(volatility>12?8:0)),5,92);
  const lossProb=100-winProb;
  const ev=round((winProb/100)*rr-(lossProb/100)*1,2);
  const quality=clamp(Math.round(score*.35+conf*.35+components.risk*.15+components.liquidity*.15),0,100);
  const maxRiskPct=quality>=85?1.25:quality>=70?1:quality>=55?.65:.35;
  return{winProb,lossProb,expectedValue:ev,signalQuality:quality,maxRiskPct:round(maxRiskPct,2),quantGrade:quality>=85?"Q1":quality>=70?"Q2":quality>=55?"Q3":"Q4"};
}
function grade({score,conf,rr,volRatio,regime,riskScore,ev}){
  if(regime.name==="CRASH_RISK")return"D";
  if(score>=84&&conf>=82&&rr>=2&&volRatio>=1.8&&riskScore>=60&&ev>1)return"A+";
  if(score>=75&&conf>=70&&rr>=1.75&&volRatio>=1.3&&riskScore>=55&&ev>.55)return"A";
  if(score>=62&&conf>=55&&ev>.15)return"B";
  if(score>=50)return"C";
  return"D";
}
function decision(g,conf,regime,ev){
...
function position(entry,sl,riskPct,capital=10000){
  const riskAmount=capital*(riskPct/100), riskPerUnit=Math.max(entry-sl,entry*.002), qty=riskAmount/riskPerUnit;
  return{capital,riskPct,riskAmount:round(riskAmount,2),qty:round(qty,6),positionValue:round(qty*entry,2)};
}
function futuresMetrics(row){
  const bias=row.grade==="A+"||row.grade==="A"?"LONG":row.grade==="D"?"SHORT/AVOID":"NEUTRAL";
  const funding=round(((row.change24h||0)/1000)+(row.volumeRatio-1)*0.0008,4);
  const oiScore=clamp(Math.round(50+row.volumeRatio*8+Math.abs(row.change24h)*2),0,100);
  const liqRisk=clamp(Math.round(row.volatility*5+(row.change24h>8?20:0)),0,100);
  const leverage=row.quant.signalQuality>=85?3:row.quant.signalQuality>=70?2:1;
  return{bias,funding,openInterestScore:oiScore,liquidationRisk:liqRisk,maxLeverage:leverage,mode:bias==="LONG"?"Long setup":bias==="SHORT/AVOID"?"Avoid long / possible short watch":"No leverage"};
}
function onchainProxy(row){
  const exchangeFlow=row.change24h>0&&row.volumeRatio>1.5?"Outflow proxy / accumulation":"Neutral flow proxy";
  const whale=row.volumeRatio>2.5&&row.change24h>0?"Whale Buy Proxy":row.volumeRatio>2&&row.change24h<0?"Whale Sell Proxy":"Normal";
  const stable=row.sector==="Major"||row.sector==="Layer1"?"High liquidity":"Medium liquidity";
  return{exchangeFlow,whale,stablecoinLiquidity:stable,onchainScore:clamp(Math.round(row.components.liquidity*.45+row.components.volume*.35+row.components.risk*.20),0,100)};
}
function backtestProxy(row){
  const expected30=round(row.quant.expectedValue*18 + row.change7d*.7,2);
  const maxDD=round(Math.max(4,row.volatility*1.35),2);
  const sharpe=round((row.quant.expectedValue+0.2)/(maxDD/20),2);
  return{expected30dPct:expected30,maxDrawdownPct:maxDD,sharpeProxy:sharpe,comment:expected30>15?"Strong historical proxy":"Moderate proxy"};
}
function summary(rows,sectors,market){
  const buy=rows.filter(x=>["A+","A"].includes(x.grade));
  const watch=rows.filter(x=>x.grade==="B");
  return{buyCount:buy.length,watchCount:watch.length,avoidCount:rows.filter(x=>["C","D"].includes(x.grade)).length,topSymbols:rows.slice(0,5).map(x=>x.symbol),bestSector:sectors[0]?.sector||"-",marketRisk:market.risk,summaryText:`ตลาด ${market.name} / Top ${rows[0]?.symbol||"-"} / Sector เด่น ${sectors[0]?.sector||"-"}`};
}
...
async function terminalPayload(limit=80){
  ...
  const avg24=avg(coins.map(x=>x.change24h)), medAbs=median(coins.map(x=>Math.abs(x.change24h)));
  const reg=marketRegime(avg24,medAbs,fng.value);
  ...
  const rows=coins.map(coin=>{
    ...
    if(reg.risk==="HIGH")penalties.push("Market Regime เสี่ยงสูง");
    if(p.rr<1.75)penalties.push("R:R ต่ำ");
    if(pack.dataQuality==="DEMO")penalties.push("ใช้ข้อมูล DemoFallback");
    ...
    row.futures=futuresMetrics(row);
    row.onchain=onchainProxy(row);
    row.backtest=backtestProxy(row);
    return row;
  }).sort((a,b)=>gradeRank(b.grade)-gradeRank(a.grade)||b.quant.signalQuality-a.quant.signalQuality||b.confidence-a.confidence);
  ...
  const terminal={
    macro:{dxyProxy:round(100-market.avg24*.35,2),nasdaqCorrelationProxy:round(clamp(0.35+market.avg24/20,0.05,0.85),2),riskMode:market.risk==="HIGH"?"Risk-Off":"Risk-On/Neutral"},
    futures:{longCandidates:rows.filter(x=>x.futures.bias==="LONG").slice(0,8).map(x=>x.symbol),highLiqRisk:rows.filter(x=>x.futures.liquidationRisk>60).slice(0,8).map(x=>x.symbol)},
    onchain:{whaleBuyProxy:rows.filter(x=>x.onchain.whale==="Whale Buy Proxy").slice(0,8).map(x=>x.symbol),exchangeOutflowProxy:rows.filter(x=>x.onchain.exchangeFlow.includes("Outflow")).slice(0,8).map(x=>x.symbol)},
    alerts:rows.filter(x=>["A+","A"].includes(x.grade)||x.quant.expectedValue>1.4).slice(0,12).map(x=>({symbol:x.symbol,type:x.grade==="A+"?"STRONG_BUY":"WATCH_BUY",message:`${x.symbol} ${x.grade} EV ${x.quant.expectedValue} Win ${x.quant.winProb}%`}))
  };
  return{ok:true,product:PRODUCT,edition:EDITION,version:API_VERSION,build:BUILD,source:pack.source,dataQuality:pack.dataQuality,cache:cacheMeta(),time:new Date().toISOString(),diagnostics:pack.diagnostics,market,sectors,summary:summary(rows,sectors,reg),terminal,topOpportunity:rows[0]||null,rows};
}

app.get("/api/version",(req,res)=>res.json({product:PRODUCT,edition:EDITION,version:VERSION,apiVersion:API_VERSION,build:BUILD,backend:"Node.js",frontend:"V12 Hedge Fund Terminal",modules:["Professional Dashboard","On-chain Proxy","Futures Engine","AI Chat","Backtest Proxy","Alerts"],status:"Production",time:new Date().toISOString()}));
...
app.get("/api/scan",async(req,res,next)=>{try{const limit=clamp(parseInt(req.query.limit||"50",10)||50,10,100);res.json(await terminalPayload(limit));}catch(e){next(e);}});
app.get("/api/terminal",async(req,res,next)=>{try{const limit=clamp(parseInt(req.query.limit||"80",10)||80,20,100);res.json(await terminalPayload(limit));}catch(e){next(e);}});
app.post("/api/ai-chat",async(req,res)=>{
  const q=String(req.body?.q||"").toUpperCase();
  const payload=await terminalPayload(60);
  const sym=(q.match(/[A-Z]{2,6}/)||[])[0];
  const row=sym?payload.rows.find(x=>x.symbol===sym):payload.topOpportunity;
  const answer=row?`${row.symbol}: Grade ${row.grade}, Score ${row.score}, Confidence ${row.confidence}%, EV ${row.quant.expectedValue}, Win ${row.quant.winProb}%. เหตุผลหลัก: ${row.reasons.join(" • ")}. Futures bias: ${row.futures.bias}. On-chain proxy: ${row.onchain.whale}. แผน: Entry ${row.entryLow}-${row.entryHigh}, SL ${row.sl}, TP1 ${row.tp1}.`:`ตลาดตอนนี้ ${payload.market.regime}, Risk ${payload.market.risk}, Sector เด่น ${payload.sectors[0]?.sector||"-"}.`;
  res.json({ok:true,version:API_VERSION,question:req.body?.q||"",answer,row:row||null,time:new Date().toISOString()});
});
