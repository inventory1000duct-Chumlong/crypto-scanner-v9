...
function loadSettings(){try{return JSON.parse(localStorage.getItem("smartQuantSettingsV32")||"{}")}catch(e){return{}}}
function saveSettings(s){localStorage.setItem("smartQuantSettingsV32",JSON.stringify(s))}
...
async function renderInstitutionalStatus(){
 ...
 `<div class="workspace-card"><h3>Build</h3><p>V32 Institutional<br>${esc(st.version||"32.0.0")}</p></div>`+
 '</div>';
}
...
function loadAudit(){try{return JSON.parse(localStorage.getItem("auditLogV32")||"[]")}catch(e){return[]}}
function saveAudit(x){localStorage.setItem("auditLogV32",JSON.stringify(x.slice(-500)))}
...
function renderFinalDashboard(){
 ...
 box.innerHTML='<div class="grid">'+
 `<div class="workspace-card"><h3>Platform</h3><p><b>V32 Final</b><br>Institutional Platform</p></div>`+
...
async function renderSystemHealth(){
 const box=$("healthV32Box"); if(!box)return;
...
function exportFinalReport(){
 const report={version:"32.0.0",time:new Date().toISOString(),settings:loadSettings(),summary:{trades:loadTrades().length,portfolio:loadPortfolio().length,watchlist:loadWatchlist().length,alerts:loadAlerts().length,plans:loadSavedPlans().length},audit:loadAudit()};
...
function renderNotificationCenter(){
 ...
 const cfg=JSON.parse(localStorage.getItem("notificationConfigV32")||"{}");
 box.innerHTML='<div class="controls">'+
 ...
 const btn=$("saveNotify"); if(btn)btn.onclick=()=>{localStorage.setItem("notificationConfigV32",JSON.stringify({webhookUrl:$("webhookUrl").value,notifyOn:$("notifyOn").value,updatedAt:new Date().toISOString()}));addAudit("Save Notification","Webhook settings updated");renderNotificationCenter();}
}
function renderApiVault(){
 const box=$("apiVaultBox"); if(!box)return;
 const cfg=JSON.parse(localStorage.getItem("apiVaultMetaV32")||"{}");
 box.innerHTML='<div class="controls">'+
 ...
 '<div class="summary"><b>Security:</b> V32 เก็บเฉพาะ metadata สำหรับทดสอบ UI เท่านั้น ไม่ควรใส่ Secret จริงใน localStorage. Production ต้องใช้ encrypted vault บน Server</div>';
 const btn=$("saveApiMeta"); if(btn)btn.onclick=()=>{localStorage.setItem("apiVaultMetaV32",JSON.stringify({exchange:$("apiExchange").value,label:$("apiKeyLabel").value,updatedAt:new Date().toISOString()}));addAudit("Save API Metadata",$("apiExchange").value);renderApiVault();}
}
function renderRoadmapFinal(){
 ...
 const items=[
  ...
  ["V32 Institutional","Enterprise dashboard, reports, audit, notifications"]
 ];
...
async function renderReleaseManager(){
 ...
 box.innerHTML='<div class="grid">'+
 `<div class="workspace-card"><h3>Current Release</h3><p><b>${esc(r.current||"V32")}</b><br>${esc(r.version||"32.0.0")}</p></div>`+
...
function exportReleaseBundle(){
 ...
 addAudit("Release Backup","V32 release backup exported");
 renderAuditLog();
}
...
async function renderDataEngine(){
 ...
 '<div class="summary"><b>V32 Data Engine:</b> ใช้ server-side JSON store เป็นฐานข้อมูลต้นแบบ และเตรียมเปลี่ยน adapter เป็น PostgreSQL ในเวอร์ชั่นถัดไป</div>';
}
...
function renderExchangeSelector(){
 ...
 const s=loadSettings();
 box.innerHTML='<div class="controls"><label>Primary Exchange<select id="exchangeSelector"><option value="aggregate">Aggregate</option><option value="coingecko">CoinGecko</option><option value="binance">Binance</option><option value="bybit">Bybit</option><option value="okx">OKX</option><option value="bitget">Bitget</option><option value="mexc">MEXC</option></select></label><button id="saveExchangeBtn">Save Exchange</button><button onclick="renderExchangeHub()">Refresh Hub</button></div><div class="summary">V32 ใช้ Adapter Framework และ Aggregator แบบพร้อมต่อ API จริง ใน Production จะเปลี่ยนจาก adapter-ready เป็น live adapters</div>';
...
async function loadQuantV32(){
 try{quantV32Payload=await api("/api/quant/v30?limit=80",30000)}catch(e){showError("V32 Quant load failed: "+e.message);quantV32Payload=null}
 renderQuantV32();
 renderEventsV32();
}
function renderQuantV32(){
 const box=$("quantV32Box"); if(!box)return;
 const p=quantV32Payload;
 if(!p){box.innerHTML='<div class="empty">ยังไม่มีข้อมูล Quant V32</div>';return}
 const q=p.quantV32||{};
 box.innerHTML='<div class="grid">'+
...
function renderEventsV32(){
 const box=$("eventsV32Box"); if(!box)return;
...
function renderMTFConsensus(){
 const box=$("mtfV32Box"); if(!box)return;
