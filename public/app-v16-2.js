...
function init(){activateTabs();$("refresh").onclick=scan;$("scanBtn").onclick=scan;$("q").oninput=renderRows;$("askAI").onclick=askAI;$("exportBtn").onclick=exportCSV; if($("addHolding"))$("addHolding").onclick=addHolding; if($("clearPortfolio"))$("clearPortfolio").onclick=clearPortfolio; if($("exportPortfolio"))$("exportPortfolio").onclick=exportPortfolioCSV; if($("addTrade"))$("addTrade").onclick=addTrade; if($("clearTrades"))$("clearTrades").onclick=clearTrades; if($("exportTrades"))$("exportTrades").onclick=exportTradesCSV; if($("backupTrades"))$("backupTrades").onclick=backupTradesJSON; if($("restoreTrades"))$("restoreTrades").onchange=e=>restoreTradesJSON(e.target.files[0]);loadFX().then(()=>loadVersion()).then(scan).catch(e=>showError(e.message)); if($("currencySelect"))$("currencySelect").onchange=()=>{renderPortfolioPL();renderJournal();}; renderHealth(); renderPortfolioPL()}
