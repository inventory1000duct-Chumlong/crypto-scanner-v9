...
function renderAll(){renderDailyDashboard();renderDailyOpportunities();renderDailyCoach();renderDailyAlerts();renderPortfolioPL();renderSettings();}
...
function loadSettings(){try{return JSON.parse(localStorage.getItem("smartQuantSettingsV20")||"{}")}catch(e){return{}}}
function saveSettings(s){localStorage.setItem("smartQuantSettingsV20",JSON.stringify(s))}
...
async function renderInstitutionalStatus(){
 ...
 `<div class="workspace-card"><h3>Build</h3><p>Daily 1.0 Institutional<br>${esc(st.version||"33.0.0")}</p></div>`+
 '</div>';
}
...
function loadAudit(){try{return JSON.parse(localStorage.getItem("auditLogV25")||"[]")}catch(e){return[]}}
function saveAudit(x){localStorage.setItem("auditLogV25",JSON.stringify(x.slice(-500)))}
...
function renderFinalDashboard(){
 ...
 box.innerHTML='<div class="grid">'+
 `<div class="workspace-card"><h3>Platform</h3><p><b>Daily 1.0 Final</b><br>Institutional Platform</p></div>`+
...
async function renderSystemHealth(){
 const box=$("healthV25Box"); if(!box)return;
...
function exportFinalReport(){
 const report={version:"33.0.0",time:new Date().toISOString(),settings:loadSettings(),summary:{trades:loadTrades().length,portfolio:loadPortfolio().length,watchlist:loadWatchlist().length,alerts:loadAlerts().length,plans:loadSavedPlans().length},audit:loadAudit()};
...
function renderNotificationCenter(){
 ...
 const cfg=JSON.parse(localStorage.getItem("notificationConfigV25")||"{}");
 box.innerHTML='<div class="controls">'+
 ...
 const btn=$("saveNotify"); if(btn)btn.onclick=()=>{localStorage.setItem("notificationConfigV25",JSON.stringify({webhookUrl:$("webhookUrl").value,notifyOn:$("notifyOn").value,updatedAt:new Date().toISOString()}));addAudit("Save Notification","Webhook settings updated");renderNotificationCenter();}
}
function renderApiVault(){
 const box=$("apiVaultBox"); if(!box)return;
 const cfg=JSON.parse(localStorage.getItem("apiVaultMetaV25")||"{}");
 box.innerHTML='<div class="controls">'+
 ...
 '<div class="summary"><b>Security:</b> Daily 1.0 เก็บเฉพาะ metadata สำหรับทดสอบ UI เท่านั้น ไม่ควรใส่ Secret จริงใน localStorage. Production ต้องใช้ encrypted vault บน Server</div>';
 const btn=$("saveApiMeta"); if(btn)btn.onclick=()=>{localStorage.setItem("apiVaultMetaV25",JSON.stringify({exchange:$("apiExchange").value,label:$("apiKeyLabel").value,updatedAt:new Date().toISOString()}));addAudit("Save API Metadata",$("apiExchange").value);renderApiVault();}
}
function renderRoadmapFinal(){
 ...
 const items=[
  ...
  ["Daily 1.0 Institutional","Enterprise dashboard, reports, audit, notifications"]
 ];
...
async function renderReleaseManager(){
 ...
 box.innerHTML='<div class="grid">'+
 `<div class="workspace-card"><h3>Current Release</h3><p><b>${esc(r.current||"Daily 1.0")}</b><br>${esc(r.version||"33.0.0")}</p></div>`+
...
function exportReleaseBundle(){
 ...
 addAudit("Release Backup","Daily 1.0 release backup exported");
 renderAuditLog();
}
...
async function renderDataEngine(){
 ...
 '<div class="summary"><b>Daily 1.0 Data Engine:</b> ใช้ server-side JSON store เป็นฐานข้อมูลต้นแบบ และเตรียมเปลี่ยน adapter เป็น PostgreSQL ในเวอร์ชั่นถัดไป</div>';
}
...
function renderExchangeSelector(){
 ...
 const s=loadSettings();
 box.innerHTML='<div class="controls"><label>Primary Exchange<select id="exchangeSelector"><option value="aggregate">Aggregate</option><option value="coingecko">CoinGecko</option><option value="binance">Binance</option><option value="bybit">Bybit</option><option value="okx">OKX</option><option value="bitget">Bitget</option><option value="mexc">MEXC</option></select></label><button id="saveExchangeBtn">Save Exchange</button><button onclick="renderExchangeHub()">Refresh Hub</button></div><div class="summary">Daily 1.0 ใช้ Adapter Framework และ Aggregator แบบพร้อมต่อ API จริง ใน Production จะเปลี่ยนจาก adapter-ready เป็น live adapters</div>';
...
let quantV32Payload=null;
async function loadQuantV30(){
 try{quantV32Payload=await api("/api/quant/v30?limit=80",30000)}catch(e){showError("Daily 1.0 Quant load failed: "+e.message);quantV32Payload=null}
 renderQuantV30();
 renderEventsV30();
}
function renderQuantV30(){
 const box=$("quantV32Box"); if(!box)return;
 const p=quantV32Payload;
 if(!p){box.innerHTML='<div class="empty">ยังไม่มีข้อมูล Quant Daily 1.0</div>';return}
 const q=p.quantV32||{};
 box.innerHTML='<div class="grid">'+
 ...
 (p.rows||[]).slice(0,40).map(x=>{const q=x.quantV32||{};return `<tr><td><b>${esc(x.symbol)}</b><br><small>${esc(x.name)}</small></td><td><b>${q.score||0}</b></td><td>${esc(q.rank||"-")}</td><td class="${probClass(q.probability||0)}">${q.probability||0}%</td><td>${fmt(q.consensus,1)}</td><td class="pnl-pos">${q.buyVotes||0}</td><td class="pnl-neg">${q.sellVotes||0}</td><td>${q.momentum||0}</td><td>${q.liquidity||0}</td><td>${(x.events||[]).length}</td></tr>`}).join("")+
 '</tbody></table>';
}
function renderEventsV30(){
 const box=$("eventsV30Box"); if(!box)return;
 const events=quantV32Payload?.quantV32?.events||[];
...
function renderMTFConsensus(){
 ...
 const rows=(quantV32Payload?.rows||[]).slice(0,30);
 const tfs=["1m","5m","15m","1h","4h","1D"];
 box.innerHTML='<table><thead><tr><th>Symbol</th>'+tfs.map(t=>`<th>${t}</th>`).join("")+'<th>Consensus</th></tr></thead><tbody>'+
 (rows.map(x=>`<tr><td><b>${esc(x.symbol)}</b></td>${tfs.map(tf=>{const m=(x.quantV32?.mtf||[]).find(a=>a.tf===tf)||{};return `<td><span class="mtf-pill ${signalClass(m.signal)}">${esc(m.signal||"-")}<br>${m.score||0}<br>RSI ${m.rsi||"-"}</span></td>`}).join("")}<td><b>${fmt(x.quantV32?.consensus,1)}</b></td></tr>`).join("")||'<tr><td colspan="8" class="empty">No MTF data</td></tr>')+
 '</tbody></table>';
}


let liveState={symbol:"BTC",timer:null,ticker:null,book:null};
async function loadLiveMarket(symbol=(liveState.symbol||chartSymbolFromPortfolio()||"BTC")){
 liveState.symbol=String(symbol||"BTC").toUpperCase();
 await Promise.all([loadLiveTicker(liveState.symbol),loadLiveBook(liveState.symbol),renderLiveStatus()]);
}
async function loadLiveTicker(symbol){
 const box=$("liveTickerBox"); if(!box)return;
 try{
   const t=await api(`/api/live/ticker/${encodeURIComponent(symbol)}`,12000);
   liveState.ticker=t;
   box.innerHTML='<div class="grid">'+
    `<div class="workspace-card"><h3>${esc(t.symbol)} Price</h3><p><b>${money(t.price,6)}</b><br>${esc(t.source)}<br>${fmt(t.change24h,2)}%</p></div>`+
    `<div class="workspace-card"><h3>Volume</h3><p>${fmt(t.volume,0)}</p></div>`+
    `<div class="workspace-card"><h3>Updated</h3><p>${new Date(t.updatedAt).toLocaleTimeString()}</p></div>`+
    `<div class="workspace-card"><h3>Mode</h3><p>Provider Fallback<br>REST Polling</p></div>`+
    '</div><h3>Provider Priority</h3><table><thead><tr><th>Priority</th><th>Provider</th><th>Status</th><th>Price</th><th>Latency</th></tr></thead><tbody>'+
    (t.providers||[]).map(p=>`<tr><td>${p.priority}</td><td><b>${esc(p.provider)}</b></td><td>${esc(p.status)}</td><td>${money(p.price,6)}</td><td>${p.latencyMs}ms</td></tr>`).join("")+
    '</tbody></table>';
 }catch(e){box.innerHTML='<div class="empty">Live ticker error: '+esc(e.message)+'</div>'}
}
async function loadLiveBook(symbol){
 const bookBox=$("orderBookBox"), tapeBox=$("tapeBox"); if(!bookBox||!tapeBox)return;
 try{
   const r=await api(`/api/live/orderbook/${encodeURIComponent(symbol)}`,12000);
   const ob=r.orderbook||{}, tape=r.tape||[];
   liveState.book=r;
   bookBox.innerHTML='<div class="grid"><div class="workspace-card"><h3>Mid</h3><p>'+money(ob.mid,6)+'</p></div><div class="workspace-card"><h3>Spread</h3><p>'+fmt(ob.spread,4)+'%</p></div><div class="workspace-card"><h3>Updated</h3><p>'+new Date(ob.updatedAt).toLocaleTimeString()+'</p></div></div>'+
   '<table><thead><tr><th>Bid Qty</th><th>Bid</th><th>Ask</th><th>Ask Qty</th></tr></thead><tbody>'+
   (ob.bids||[]).map((b,i)=>{const a=(ob.asks||[])[i]||{};return `<tr><td class="pnl-pos">${fmt(b.qty,4)}</td><td class="pnl-pos">${money(b.price,6)}</td><td class="pnl-neg">${money(a.price,6)}</td><td class="pnl-neg">${fmt(a.qty,4)}</td></tr>`}).join("")+'</tbody></table>';
   tapeBox.innerHTML='<table><thead><tr><th>Time</th><th>Side</th><th>Price</th><th>Qty</th><th>Value</th></tr></thead><tbody>'+
   tape.map(t=>`<tr><td>${new Date(t.time).toLocaleTimeString()}</td><td class="${t.side==='BUY'?'pnl-pos':'pnl-neg'}"><b>${t.side}</b></td><td>${money(t.price,6)}</td><td>${fmt(t.qty,4)}</td><td>${money(t.value,2)}</td></tr>`).join("")+'</tbody></table>';
 }catch(e){bookBox.innerHTML='<div class="empty">Orderbook error: '+esc(e.message)+'</div>'}
}
async function renderLiveStatus(){
 const box=$("liveStatusBox"); if(!box)return;
 try{
   const s=await api("/api/live/status",12000);
   box.innerHTML='<div class="grid">'+
    `<div class="workspace-card"><h3>Engine</h3><p>${s.ok?'✅ ONLINE':'❌ OFFLINE'}<br>${esc(s.engine)}</p></div>`+
    `<div class="workspace-card"><h3>Mode</h3><p>${esc(s.mode)}<br>WebSocket ${esc(s.websocket)}</p></div>`+
    `<div class="workspace-card"><h3>CoinGecko</h3><p>${esc(s.coinGecko?.status||"-")}<br>Age ${s.coinGecko?.ageSec||0}s</p></div>`+
    `<div class="workspace-card"><h3>Latency</h3><p>${s.latencyMs}ms<br>${esc(s.dataQuality)}</p></div>`+
    '</div>';
 }catch(e){box.innerHTML='<div class="empty">Live status error: '+esc(e.message)+'</div>'}
}
function toggleLiveEngine(){
 if(liveState.timer){clearInterval(liveState.timer);liveState.timer=null;showError("Live Engine OFF");return}
 liveState.timer=setInterval(()=>loadLiveMarket(liveState.symbol||"BTC"),5000);
 loadLiveMarket(liveState.symbol||"BTC");
}
function renderLiveMarketPane(){
 const box=$("liveTickerBox"); if(!box)return;
 if(!liveState.ticker)loadLiveMarket(liveState.symbol||chartSymbolFromPortfolio()||"BTC");
}


let streamState={symbol:"BTC",tf:"1m",timer:null};
async function renderStreamStatus(){
 const box=$("streamStatusBox"); if(!box)return;
 try{
   const s=await api("/api/stream/status",12000);
   box.innerHTML='<div class="grid">'+
    `<div class="workspace-card"><h3>Stream Engine</h3><p>${s.ok?'✅ ONLINE':'❌ OFFLINE'}<br>${esc(s.mode)}</p></div>`+
    `<div class="workspace-card"><h3>Ticks</h3><p>${s.lastTickCount||0} buffered<br>${s.symbols||0} symbols</p></div>`+
    `<div class="workspace-card"><h3>Market Symbols</h3><p>${s.marketSymbols||0}</p></div>`+
    `<div class="workspace-card"><h3>Started</h3><p>${new Date(s.startedAt).toLocaleTimeString()}</p></div>`+
    '</div><h3>Provider Streams</h3><table><thead><tr><th>Provider</th><th>Status</th><th>Latency</th><th>WebSocket</th></tr></thead><tbody>'+
    (s.providers||[]).map(p=>`<tr><td><b>${esc(p.id)}</b></td><td>${esc(p.status)}</td><td>${p.latencyMs}ms</td><td>${p.wsReady?'ready':'active simulator'}</td></tr>`).join("")+
    '</tbody></table>';
 }catch(e){box.innerHTML='<div class="empty">Stream status error: '+esc(e.message)+'</div>'}
}
async function renderTickBuffer(symbol=streamState.symbol){
 const box=$("tickBufferBox"); if(!box)return;
 streamState.symbol=String(symbol||"BTC").toUpperCase();
 try{
   const r=await api(`/api/stream/ticks/${encodeURIComponent(streamState.symbol)}?count=8`,12000);
   const ticks=(r.ticks||[]).slice(-40).reverse();
   box.innerHTML='<div class="grid">'+
    `<div class="workspace-card"><h3>${esc(r.symbol)} Last Tick</h3><p>${money(r.last?.price,6)}<br>${esc(r.last?.side||"-")}</p></div>`+
    `<div class="workspace-card"><h3>Buffer</h3><p>${(r.ticks||[]).length} ticks</p></div>`+
    `<div class="workspace-card"><h3>Updated</h3><p>${new Date(r.time).toLocaleTimeString()}</p></div>`+
    '</div><table><thead><tr><th>Time</th><th>Side</th><th>Price</th><th>Qty</th><th>Value</th></tr></thead><tbody>'+
    ticks.map(t=>`<tr><td>${new Date(t.time).toLocaleTimeString()}</td><td class="${t.side==='BUY'?'pnl-pos':'pnl-neg'}"><b>${t.side}</b></td><td>${money(t.price,6)}</td><td>${fmt(t.qty,5)}</td><td>${money(t.value,2)}</td></tr>`).join("")+
    '</tbody></table>';
 }catch(e){box.innerHTML='<div class="empty">Tick error: '+esc(e.message)+'</div>'}
}
async function renderStreamCandles(symbol=streamState.symbol,tf=streamState.tf){
 const box=$("streamCandleBox"); if(!box)return;
 streamState.symbol=String(symbol||"BTC").toUpperCase();streamState.tf=tf||"1m";
 try{
   const r=await api(`/api/stream/candles/${encodeURIComponent(streamState.symbol)}?tf=${encodeURIComponent(streamState.tf)}`,15000);
   const last=r.last?.price||0;
   const levels={entry:last,sl:last*0.985,tp1:last*1.012,tp2:last*1.025,tp3:last*1.04};
   box.innerHTML='<div class="chart-title-row"><div><h3>'+esc(r.symbol)+' Stream Candles <small>'+esc(r.tf)+'</small></h3><p>'+esc(r.note||"")+'</p></div><div class="chart-live-badge">STREAM</div></div>'+drawRealtimeSVG(r.candles,levels,r.symbol);
 }catch(e){box.innerHTML='<div class="empty">Candle stream error: '+esc(e.message)+'</div>'}
}
function toggleStreamEngine(){
 if(streamState.timer){clearInterval(streamState.timer);streamState.timer=null;showError("Stream Engine OFF");return}
 streamState.timer=setInterval(()=>{renderTickBuffer(streamState.symbol);renderStreamCandles(streamState.symbol,streamState.tf);renderStreamStatus()},3000);
 renderTickBuffer(streamState.symbol);renderStreamCandles(streamState.symbol,streamState.tf);renderStreamStatus();
}
function renderStreamPane(){
 const box=$("streamStatusBox"); if(!box)return;
 renderStreamStatus();renderTickBuffer(streamState.symbol);renderStreamCandles(streamState.symbol,streamState.tf);
}


let dailyPayload=null;
async function loadDaily(){
 try{dailyPayload=await api("/api/daily",30000)}catch(e){showError("Daily load failed: "+e.message);dailyPayload=null}
 renderDailyDashboard();renderDailyOpportunities();renderDailyCoach();renderDailyAlerts();
}
function decisionBadge(action){
 const m={BUY_ZONE:"เข้าโซน",WATCH:"รอจังหวะ",WAIT:"รอ",AVOID:"เลี่ยง",HOLD:"ถือ"};
 return `<span class="badge ${action==='BUY_ZONE'?'buy':action==='AVOID'?'sell':'hold'}">${m[action]||action}</span>`;
}
function renderDailyDashboard(){
 const box=$("dailyDashboardBox"); if(!box)return;
 const d=dailyPayload?.daily;
 if(!d){box.innerHTML='<div class="empty">กำลังโหลด Daily Dashboard...</div>';return}
 const p=portfolioRows();
 const totalCost=p.reduce((s,x)=>s+x.cost,0), totalValue=p.reduce((s,x)=>s+x.value,0), pnl=totalValue-totalCost, pnlPct=totalCost?pnl/totalCost*100:0;
 box.innerHTML='<div class="daily-hero">'+
 `<div><h2>สวัสดีครับ วันนี้ตลาดพร้อมแค่ไหน?</h2><div class="daily-score">${d.marketScore}/100</div><p>${esc(d.brief.join(" • "))}</p></div>`+
 `<div class="daily-side"><b>Fear & Greed</b><span>${dailyPayload.market?.fng??"-"}</span><b>Risk</b><span>${esc(d.riskMode)}</span></div>`+
 '</div><div class="journal-kpis">'+
 `<div class="journal-kpi"><span>Opportunities</span><b>${d.opportunities.length}</b></div>`+
 `<div class="journal-kpi"><span>Portfolio Value</span><b>${money(totalValue,2)}</b></div>`+
 `<div class="journal-kpi"><span>Portfolio P/L</span><b class="${pnl>=0?'pnl-pos':'pnl-neg'}">${money(pnl,2)}</b></div>`+
 `<div class="journal-kpi"><span>ROI</span><b class="${pnl>=0?'pnl-pos':'pnl-neg'}">${fmt(pnlPct,2)}%</b></div>`+
 `<div class="journal-kpi"><span>Updated</span><b>${new Date(dailyPayload.time).toLocaleTimeString()}</b></div>`+
 '</div>';
}
function renderDailyOpportunities(){
 const box=$("dailyOppBox"); if(!box)return;
 const ops=dailyPayload?.daily?.opportunities||[];
 if(!ops.length){box.innerHTML='<div class="empty">วันนี้ยังไม่มีเหรียญเข้าเกณฑ์</div>';return}
 box.innerHTML='<table><thead><tr><th>เหรียญ</th><th>Decision</th><th>Score</th><th>Prob</th><th>ราคา</th><th>Entry</th><th>SL</th><th>TP1/TP2</th><th>เหตุผล</th></tr></thead><tbody>'+
 ops.map(x=>`<tr><td><b>${esc(x.symbol)}</b><br><small>${esc(x.name)}</small></td><td>${decisionBadge(x.daily.action)}</td><td><b>${x.daily.score}</b></td><td>${x.daily.probability}%</td><td>${money(x.price,4)}<br><small>${fmt(x.change24h,2)}%</small></td><td>${money(x.entry,4)}</td><td class="pnl-neg">${money(x.sl,4)}</td><td class="pnl-pos">${money(x.tp1,4)} / ${money(x.tp2,4)}</td><td>${esc(x.daily.reason)}</td></tr>`).join("")+
 '</tbody></table>';
}
function renderDailyCoach(){
 const box=$("dailyCoachBox"); if(!box)return;
 const d=dailyPayload?.daily;
 if(!d){box.innerHTML='<div class="empty">กำลังโหลด AI Coach...</div>';return}
 const buy=d.opportunities.filter(x=>x.daily.action==="BUY_ZONE").slice(0,5);
 const watch=d.opportunities.filter(x=>x.daily.action==="WATCH").slice(0,5);
 const avoid=d.avoid.slice(0,5);
 const advice=[];
 advice.push(`ตลาดวันนี้ได้คะแนน ${d.marketScore}/100 ความเสี่ยง ${d.riskMode}`);
 if(buy.length)advice.push(`เหรียญที่เข้าโซนน่าสนใจ: ${buy.map(x=>x.symbol).join(", ")}`);
 if(watch.length)advice.push(`เหรียญที่ควรรอจังหวะ: ${watch.map(x=>x.symbol).join(", ")}`);
 if(avoid.length)advice.push(`หลีกเลี่ยง/ยังไม่ชัด: ${avoid.map(x=>x.symbol).join(", ")}`);
 advice.push(d.riskMode==="HIGH"?"ใช้เงินต่อไม้ให้น้อยลง และรอ Confirm เพิ่ม":"ถ้าจะเข้า ให้ยึด Entry / SL / TP ตามแผนเท่านั้น");
 box.innerHTML='<div class="coach-card"><h3>AI Coach — สรุป 30 วินาที</h3><ol>'+advice.map(x=>`<li>${esc(x)}</li>`).join("")+'</ol></div>';
}
function renderDailyAlerts(){
 const box=$("dailyAlertsBox"); if(!box)return;
 const ops=dailyPayload?.daily?.opportunities||[];
 const alerts=[];
 for(const x of ops){
   if(Math.abs(x.change24h)>6)alerts.push({time:new Date().toLocaleTimeString(),symbol:x.symbol,type:"Price Move",msg:`เคลื่อนไหว ${fmt(x.change24h,2)}%`});
   if(x.daily.eventCount>0)alerts.push({time:new Date().toLocaleTimeString(),symbol:x.symbol,type:"Event",msg:`มี Event ${x.daily.eventCount} รายการ`});
   if(x.daily.action==="BUY_ZONE")alerts.push({time:new Date().toLocaleTimeString(),symbol:x.symbol,type:"Buy Zone",msg:"เข้าเกณฑ์พิจารณาเข้าซื้อเมื่อราคาอยู่ในแผน"});
 }
 box.innerHTML='<table><thead><tr><th>เวลา</th><th>เหรียญ</th><th>ประเภท</th><th>รายละเอียด</th></tr></thead><tbody>'+
 (alerts.slice(0,30).map(a=>`<tr><td>${esc(a.time)}</td><td><b>${esc(a.symbol)}</b></td><td>${esc(a.type)}</td><td>${esc(a.msg)}</td></tr>`).join("")||'<tr><td colspan="4" class="empty">ยังไม่มี Alert สำคัญ</td></tr>')+
 '</tbody></table>';
}
function renderDeveloperMode(){
 const box=$("developerModeBox"); if(!box)return;
 box.innerHTML='<details><summary>Developer Mode / Advanced APIs</summary><div class="summary">API เดิมยังอยู่ เช่น /api/scan, /api/chart/BTC, /api/daily, /api/version</div></details>';
}
...
function renderMorningDashboard(){
  ...
  if(!p)return;
  const status=$("morningStatus");
  if(status) status.textContent="อัปเดตล่าสุด "+new Date(p.time).toLocaleTimeString();
  ...
  $("marketScore").textContent=(p.summary?.marketScore||0)+"/100";
  $("fearGreed").textContent=p.market?.fng??"-";
  $("marketRisk").textContent=p.market?.risk||"-";
  $("oppCount").textContent=p.summary?.totalOpportunities||0;
  $("portfolioValue").textContent=money(totalValue,2);
  $("portfolioPL").innerHTML=`<span class="${pnl>=0?'pnl-pos':'pnl-neg'}">${money(pnl,2)} (${fmt(roi,2)}%)</span>`;

  const ops=p.opportunities||[];
  $("topOppBox").innerHTML=ops.slice(0,10).map((x,i)=>`
  ...
  $("alertsBox").innerHTML=(p.alerts||[]).slice(0,10).map(a=>`
...
function addMorningPortfolioHooks(){
  if($("addPortfolio"))$("addPortfolio").onclick=()=>{addPortfolio();setTimeout(renderMorningDashboard,300)};
  if($("clearPortfolio"))$("clearPortfolio").onclick=()=>{clearPortfolio();setTimeout(renderMorningDashboard,300)};
  if($("exportPortfolio"))$("exportPortfolio").onclick=exportPortfolioCSV;
}

function renderAll(){renderMorningDashboard();renderPortfolioPL();}
setTimeout(()=>{loadMorning();addMorningPortfolioHooks();},800);
