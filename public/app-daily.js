...
function renderAll(){renderDailyDashboard();renderDailyOpportunities();renderDailyCoach();renderDailyAlerts();renderDeveloperMode();renderSummary();renderPick();renderRows();renderPortfolioPL();renderAlerts();renderSettings();}
...
async function renderInstitutionalStatus(){
 ...
 `<div class="workspace-card"><h3>Build</h3><p>Daily 1.0 Institutional<br>${esc(st.version||"1.0.0")}</p></div>`+
 '</div>';
}
...
function exportFinalReport(){
 const report={version:"1.0.0",time:new Date().toISOString(),settings:loadSettings(),summary:{trades:loadTrades().length,portfolio:loadPortfolio().length,watchlist:loadWatchlist().length,alerts:loadAlerts().length,plans:loadSavedPlans().length},audit:loadAudit()};
...
async function renderReleaseManager(){
 ...
 box.innerHTML='<div class="grid">'+
 `<div class="workspace-card"><h3>Current Release</h3><p><b>${esc(r.current||"Daily 1.0")}</b><br>${esc(r.version||"1.0.0")}</p></div>`+
