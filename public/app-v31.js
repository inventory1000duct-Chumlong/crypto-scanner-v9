...
function loadSettings(){try{return JSON.parse(localStorage.getItem("smartQuantSettingsV31")||"{}")}catch(e){return{}}}
function saveSettings(s){localStorage.setItem("smartQuantSettingsV31",JSON.stringify(s))}
...
async function renderInstitutionalStatus(){
 ...
 `<div class="workspace-card"><h3>Build</h3><p>V31 Institutional<br>${esc(st.version||"31.0.0")}</p></div>`+
 '</div>';
}
...
function loadAudit(){try{return JSON.parse(localStorage.getItem("auditLogV31")||"[]")}catch(e){return[]}}
function saveAudit(x){localStorage.setItem("auditLogV31",JSON.stringify(x.slice(-500)))}
...
function renderFinalDashboard(){
 ...
 box.innerHTML='<div class="grid">'+
 `<div class="workspace-card"><h3>Platform</h3><p><b>V31 Final</b><br>Institutional Platform</p></div>`+
...
async function renderSystemHealth(){
 const box=$("healthV31Box"); if(!box)return;
...
function exportFinalReport(){
 const report={version:"31.0.0",time:new Date().toISOString(),settings:loadSettings(),summary:{trades:loadTrades().length,portfolio:loadPortfolio().length,watchlist:loadWatchlist().length,alerts:loadAlerts().length,plans:loadSavedPlans().length},audit:loadAudit()};
...
function renderNotificationCenter(){
 ...
 const cfg=JSON.parse(localStorage.getItem("notificationConfigV31")||"{}");
 box.innerHTML='<div class="controls">'+
 ...
 const btn=$("saveNotify"); if(btn)btn.onclick=()=>{localStorage.setItem("notificationConfigV31",JSON.stringify({webhookUrl:$("webhookUrl").value,notifyOn:$("notifyOn").value,updatedAt:new Date().toISOString()}));addAudit("Save Notification","Webhook settings updated");renderNotificationCenter();}
}
function renderApiVault(){
 const box=$("apiVaultBox"); if(!box)return;
 const cfg=JSON.parse(localStorage.getItem("apiVaultMetaV31")||"{}");
 box.innerHTML='<div class="controls">'+
 ...
 '<div class="summary"><b>Security:</b> V31 เก็บเฉพาะ metadata สำหรับทดสอบ UI เท่านั้น ไม่ควรใส่ Secret จริงใน localStorage. Production ต้องใช้ encrypted vault บน Server</div>';
 const btn=$("saveApiMeta"); if(btn)btn.onclick=()=>{localStorage.setItem("apiVaultMetaV31",JSON.stringify({exchange:$("apiExchange").value,label:$("apiKeyLabel").value,updatedAt:new Date().toISOString()}));addAudit("Save API Metadata",$("apiExchange").value);renderApiVault();}
}
function renderRoadmapFinal(){
 ...
 const items=[
  ...
  ["V31 Institutional","Enterprise dashboard, reports, audit, notifications"]
 ];
...
async function renderReleaseManager(){
 ...
 box.innerHTML='<div class="grid">'+
 `<div class="workspace-card"><h3>Current Release</h3><p><b>${esc(r.current||"V31")}</b><br>${esc(r.version||"31.0.0")}</p></div>`+
...
function exportReleaseBundle(){
 ...
 addAudit("Release Backup","V31 release backup exported");
 renderAuditLog();
}
...
async function renderDataEngine(){
 ...
 '<div class="summary"><b>V31 Data Engine:</b> ใช้ server-side JSON store เป็นฐานข้อมูลต้นแบบ และเตรียมเปลี่ยน adapter เป็น PostgreSQL ในเวอร์ชั่นถัดไป</div>';
}
...
function renderExchangeSelector(){
 ...
 const s=loadSettings();
 box.innerHTML='<div class="controls"><label>Primary Exchange<select id="exchangeSelector"><option value="aggregate">Aggregate</option><option value="coingecko">CoinGecko</option><option value="binance">Binance</option><option value="bybit">Bybit</option><option value="okx">OKX</option><option value="bitget">Bitget</option><option value="mexc">MEXC</option></select></label><button id="saveExchangeBtn">Save Exchange</button><button onclick="renderExchangeHub()">Refresh Hub</button></div><div class="summary">V31 ใช้ Adapter Framework และ Aggregator แบบพร้อมต่อ API จริง ใน Production จะเปลี่ยนจาก adapter-ready เป็น live adapters</div>';
...
let quantV31Payload=null;
async function loadQuantV31(){
 try{quantV31Payload=await api("/api/quant/v30?limit=80",30000)}catch(e){showError("V31 Quant load failed: "+e.message);quantV31Payload=null}
 renderQuantV31();
 renderEventsV31();
}
function renderQuantV31(){
 const box=$("quantV31Box"); if(!box)return;
 const p=quantV31Payload;
 if(!p){box.innerHTML='<div class="empty">ยังไม่มีข้อมูล Quant V31</div>';return}
 const q=p.quantV31||{};
 box.innerHTML='<div class="grid">'+
 ...
 (p.rows||[]).slice(0,40).map(x=>{const q=x.quantV31||{};return `<tr><td><b>${esc(x.symbol)}</b><br><small>${esc(x.name)}</small></td><td><b>${q.score||0}</b></td><td>${esc(q.rank||"-")}</td><td class="${probClass(q.probability||0)}">${q.probability||0}%</td><td>${fmt(q.consensus,1)}</td><td class="pnl-pos">${q.buyVotes||0}</td><td class="pnl-neg">${q.sellVotes||0}</td><td>${q.momentum||0}</td><td>${q.liquidity||0}</td><td>${(x.events||[]).length}</td></tr>`}).join("")+
 '</tbody></table>';
}
function renderEventsV31(){
 const box=$("eventsV31Box"); if(!box)return;
 const events=quantV31Payload?.quantV31?.events||[];
...
function renderMTFConsensus(){
 const box=$("mtfV31Box"); if(!box)return;
 const rows=(quantV31Payload?.rows||[]).slice(0,30);
 const tfs=["1m","5m","15m","1h","4h","1D"];
 box.innerHTML='<table><thead><tr><th>Symbol</th>'+tfs.map(t=>`<th>${t}</th>`).join("")+'<th>Consensus</th></tr></thead><tbody>'+
 (rows.map(x=>`<tr><td><b>${esc(x.symbol)}</b></td>${tfs.map(tf=>{const m=(x.quantV31?.mtf||[]).find(a=>a.tf===tf)||{};return `<td><span class="mtf-pill ${signalClass(m.signal)}">${esc(m.signal||"-")}<br>${m.score||0}<br>RSI ${m.rsi||"-"}</span></td>`}).join("")}<td><b>${fmt(x.quantV31?.consensus,1)}</b></td></tr>`).join("")||'<tr><td colspan="8" class="empty">No MTF data</td></tr>')+
 '</tbody></table>';
}
