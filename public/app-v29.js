...
function loadSettings(){try{return JSON.parse(localStorage.getItem("smartQuantSettingsV29")||"{}")}catch(e){return{}}}
function saveSettings(s){localStorage.setItem("smartQuantSettingsV29",JSON.stringify(s))}
...
async function renderInstitutionalStatus(){
 ...
 `<div class="workspace-card"><h3>Build</h3><p>V29 Institutional<br>${esc(st.version||"29.0.0")}</p></div>`+
 '</div>';
}
...
function loadAudit(){try{return JSON.parse(localStorage.getItem("auditLogV29")||"[]")}catch(e){return[]}}
function saveAudit(x){localStorage.setItem("auditLogV29",JSON.stringify(x.slice(-500)))}
...
function renderFinalDashboard(){
 ...
 box.innerHTML='<div class="grid">'+
 `<div class="workspace-card"><h3>Platform</h3><p><b>V29 Final</b><br>Institutional Platform</p></div>`+
...
async function renderSystemHealth(){
 const box=$("healthV29Box"); if(!box)return;
...
function exportFinalReport(){
 const report={version:"29.0.0",time:new Date().toISOString(),settings:loadSettings(),summary:{trades:loadTrades().length,portfolio:loadPortfolio().length,watchlist:loadWatchlist().length,alerts:loadAlerts().length,plans:loadSavedPlans().length},audit:loadAudit()};
...
function renderNotificationCenter(){
 ...
 const cfg=JSON.parse(localStorage.getItem("notificationConfigV29")||"{}");
 box.innerHTML='<div class="controls">'+
 ...
 const btn=$("saveNotify"); if(btn)btn.onclick=()=>{localStorage.setItem("notificationConfigV29",JSON.stringify({webhookUrl:$("webhookUrl").value,notifyOn:$("notifyOn").value,updatedAt:new Date().toISOString()}));addAudit("Save Notification","Webhook settings updated");renderNotificationCenter();}
}
function renderApiVault(){
 const box=$("apiVaultBox"); if(!box)return;
 const cfg=JSON.parse(localStorage.getItem("apiVaultMetaV29")||"{}");
 box.innerHTML='<div class="controls">'+
 ...
 '<div class="summary"><b>Security:</b> V29 เก็บเฉพาะ metadata สำหรับทดสอบ UI เท่านั้น ไม่ควรใส่ Secret จริงใน localStorage. Production ต้องใช้ encrypted vault บน Server</div>';
 const btn=$("saveApiMeta"); if(btn)btn.onclick=()=>{localStorage.setItem("apiVaultMetaV29",JSON.stringify({exchange:$("apiExchange").value,label:$("apiKeyLabel").value,updatedAt:new Date().toISOString()}));addAudit("Save API Metadata",$("apiExchange").value);renderApiVault();}
}
function renderRoadmapFinal(){
 ...
 const items=[
  ...
  ["V29 Institutional","Enterprise dashboard, reports, audit, notifications"]
 ];
...
async function renderReleaseManager(){
 ...
 box.innerHTML='<div class="grid">'+
 `<div class="workspace-card"><h3>Current Release</h3><p><b>${esc(r.current||"V29")}</b><br>${esc(r.version||"29.0.0")}</p></div>`+
...
function exportReleaseBundle(){
 ...
 addAudit("Release Backup","V29 release backup exported");
 renderAuditLog();
}
...
async function renderDataEngine(){
 ...
 '<div class="summary"><b>V29 Data Engine:</b> ใช้ server-side JSON store เป็นฐานข้อมูลต้นแบบ และเตรียมเปลี่ยน adapter เป็น PostgreSQL ในเวอร์ชั่นถัดไป</div>';
}
...
function renderExchangeSelector(){
 ...
 const s=loadSettings();
 box.innerHTML='<div class="controls"><label>Primary Exchange<select id="exchangeSelector"><option value="aggregate">Aggregate</option><option value="coingecko">CoinGecko</option><option value="binance">Binance</option><option value="bybit">Bybit</option><option value="okx">OKX</option><option value="bitget">Bitget</option><option value="mexc">MEXC</option></select></label><button id="saveExchangeBtn">Save Exchange</button><button onclick="renderExchangeHub()">Refresh Hub</button></div><div class="summary">V29 ใช้ Adapter Framework และ Aggregator แบบพร้อมต่อ API จริง ใน Production จะเปลี่ยนจาก adapter-ready เป็น live adapters</div>';
