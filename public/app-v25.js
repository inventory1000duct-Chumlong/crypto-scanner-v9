...
function loadSettings(){try{return JSON.parse(localStorage.getItem("smartQuantSettingsV25")||"{}")}catch(e){return{}}}
function saveSettings(s){localStorage.setItem("smartQuantSettingsV25",JSON.stringify(s))}
...
async function renderInstitutionalStatus(){
 ...
 `<div class="workspace-card"><h3>Build</h3><p>V25 Institutional<br>${esc(st.version||"25.0.0")}</p></div>`+
 '</div>';
}
...
function renderFinalDashboard(){
 ...
 box.innerHTML='<div class="grid">'+
 `<div class="workspace-card"><h3>Platform</h3><p><b>V25 Final</b><br>Institutional Platform</p></div>`+
...
function exportFinalReport(){
 const report={version:"25.0.0",time:new Date().toISOString(),settings:loadSettings(),summary:{trades:loadTrades().length,portfolio:loadPortfolio().length,watchlist:loadWatchlist().length,alerts:loadAlerts().length,plans:loadSavedPlans().length},audit:loadAudit()};
...
function renderApiVault(){
 ...
 '<div class="summary"><b>Security:</b> V25 เก็บเฉพาะ metadata สำหรับทดสอบ UI เท่านั้น ไม่ควรใส่ Secret จริงใน localStorage. Production ต้องใช้ encrypted vault บน Server</div>';
...
function renderRoadmapFinal(){
 ...
 const items=[
  ...
  ["V25 Institutional","Enterprise dashboard, reports, audit, notifications"]
 ];
